...
class ZappaCLI:
    ...
    def deploy(self, source_zip=None, docker_image_uri=None):
        ...
        if self.use_apigateway:
            ...
            api_id = self.zappa.get_api_id(
                self.lambda_name, apigateway_version=self.apigateway_version
            )
            ...
            if (
                self.stage_config.get("payload_compression", True)
                and self.apigateway_version == "v1"
            ):
    ...
    def undeploy(self, no_confirm=False, remove_logs=False):
        ...
        if self.use_apigateway:
            ...
            self.zappa.undeploy_api_gateway(
    ...
    def status(self, return_json=False):
        ...
        if self.use_function_url:
            try:
                response = self.zappa.lambda_client.list_function_url_configs(
                    FunctionName=self.lambda_name, MaxItems=50
                )
                if response.get("FunctionUrlConfigs"):
                    status_dict["Function URL"] = response["FunctionUrlConfigs"][0][
                        "FunctionUrl"
                    ]
                else:
    ...
    def _get_init_profile(
        self, default_profile: str, profiles: dict, profile_names: list
    ) -> tuple[str, dict]:
        while True:
            profile_name = (
                input(
...
╚══════╝╚═╝  ╚═╝╚═╝     ╚═╝     ╚═╝  ╚═╝\n""",
        ...
        if not profile_names:
            ...
            profile_name, profile = self._get_init_profile(
                default_profile, profiles, profile_names
            )
    ...
    def callback(self, position):
        ...
        if callback:
            ...
            try:  # Prefer callback in working directory
                ...
                module_ = working_dir_importer.find_spec(mod_name).loader.load_module(
                    mod_name
                )
    ...
    def load_settings(self, settings_file=None, session=None):
        ...
        if "Size" not in self.ephemeral_storage:
            raise ClickException(
                "Please provide a valid Size for ephemeral_storage in your Zappa settings."
            )
        elif not 512 <= self.ephemeral_storage["Size"] <= 10240:
            raise ClickException(
                "Please provide a valid ephemeral_storage size between 512 - 10240 in your Zappa settings."
            )
        ...
        self.additional_text_mimetypes = self.stage_config.get(
            "additional_text_mimetypes", None
        )
        ...
        if self.additional_text_mimetypes and not self.binary_support:
            raise ClickException(
                "zappa_settings.json has additional_text_mimetypes defined, but binary_support is False!"
            )
        ...
        self.function_url_cloudfront_config = self.stage_config.get(
            "function_url_cloudfront_config", {}
        )
        ...
        default_function_url_config.update(
            self.stage_config.get("function_url_config", {})
        )
    ...
    def get_zappa_settings_string(self):
        ...
        for bot_event in bot_events:
            ...
            if intent and invocation_source and function:
                bot_events_mapping[str(intent) + ":" + str(invocation_source)] = (
                    function
                )
        ...
        additional_text_mimetypes = self.stage_config.get(
            "additional_text_mimetypes", []
        )
